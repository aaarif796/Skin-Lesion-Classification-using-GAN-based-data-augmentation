...
max_cap = 300
# Group by 'label_encoded' and sample up to max_cap rows from each group
df = df.groupby('dx').apply(lambda x: x.sample(n=min(len(x), max_cap))).reset_index(drop=True)
...
gen_checkpoint = ModelCheckpoint(filepath='best_models/best_generator.h5', 
                                 monitor='gen_loss', 
                                 save_best_only=True, 
...
disc_checkpoint = ModelCheckpoint(filepath='best_models/best_discriminator.h5', 
                                  monitor='disc_loss', 
                                  save_best_only=True, 
                                  save_weights_only=True, 
                                  mode='min', 
...
with tf.device("/GPU:0"):
    for epoch in tqdm(range(Epochs)):
        ...
        if (epoch + 1) % 100 == 0:
            gen_checkpoint_path = f'checkpoints/generator_epoch_{epoch + 1}.h5'
            disc_checkpoint_path = f'checkpoints/discriminator_epoch_{epoch + 1}.h5'
...
with tf.device("/GPU:0"):
    for epoch in tqdm(range(Epochs)):
        ...
        # Save the generator and discriminator models if they improve
        gen_checkpoint.model = generator
        disc_checkpoint.model = discriminator
        
        # Manually call the checkpoint save function
        gen_checkpoint.on_epoch_end(epoch, logs={'gen_loss': gen_losses[-1]})
        disc_checkpoint.on_epoch_end(epoch, logs={'disc_loss': disc_losses[-1]})

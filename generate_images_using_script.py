...
generator.load_weights('checkpoints/generator_epoch_500.h5')  # Load the best saved weights
...
def generate_single_image():
    ...
    file_path = os.path.join(folder, 'single_generated_image_500ep.png')
...
def generate_images_for_class_0(n):
    # Create folder if it doesn't exist
    folder = 'generated_images_class_0_akiec'
    if not os.path.exists(folder):
        os.makedirs(folder)
    ...
    for i in range(n):
        ...
        file_path = os.path.join(folder, f'generated_image_class_0_{i+1}.jpg')
        img = Image.fromarray(generated_image)
        img.save(file_path)

        print(f"Image {i+1} for class 0 saved to {file_path}")
generate_images_for_class_0(1000)


# Function to generate `n` images for class 0 and save them
def generate_images_for_class_1(n):
    # Create folder if it doesn't exist
    folder = 'generated_images_class_1_bcc'
    if not os.path.exists(folder):
        os.makedirs(folder)
    ...
    for i in range(n):
        ...
        # Save the generated image for class 0
        file_path = os.path.join(folder, f'generated_image_class_1_{i+1}.jpg')
        img = Image.fromarray(generated_image)
        img.save(file_path)

        print(f"Image {i+1} for class 1 saved to {file_path}")
        
generate_images_for_class_1(1000)
...
def generate_images_for_class_3(n):
    # Create folder if it doesn't exist
    folder = 'generated_images_class_3_df'
    if not os.path.exists(folder):
        os.makedirs(folder)

    for i in range(n):
        noise = tf.random.normal([1, Z_DIM])  # Generate random noise for one image
        label_tensor = tf.convert_to_tensor([3])  # Class 0
        ...
        # Save the generated image for class 0
        file_path = os.path.join(folder, f'generated_image_class_3_{i+1}.jpg')
        img = Image.fromarray(generated_image)
        img.save(file_path)
...
n_images = 1000  # Change this number to generate more or fewer images
...
def generate_images_for_class_6(n):
    # Create folder if it doesn't exist
    folder = 'generated_images_class_6_vasc'
    if not os.path.exists(folder):
        os.makedirs(folder)
